...
<body>
    ...
    <h1 style="margin: 50px 50px">Vấn đề mới</h1>
    <form action="{{ route('issues.store') }}" method="POST" style="margin: 50px 50px">
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="computer_id" name="computer_id" required>
                @foreach($computers as $computer)
                    <option value="{{ $computer->id }}" {{ $computer->id == old('computer_id', $issue->computer_id ?? '') ? 'selected' : '' }}>
                        {{ $computer->computer_name }} - {{ $computer->operating_system }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            <label for="reported_by" class="form-label">Người báo sự cố</label>
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" id="reported_date" name="reported_date" rows="3" required></input>
        </div>
        <div class=" mb-3">
